...
2. 必须包含以下核心信息：
   ...
   - 明确说明：**AI需要负责剧情的推进**
   - 说明使用第一人称还是第三人称书写
   - **根据世界观设定，推断并说明作品的题材和文风**（例如：中国都市、仙侠、武侠、异世界、古装、现代都市、轻小说、西方幻想、校园等）
3. **重要警告**：绝对不能搞反角色分配！
...
正确格式示例（但措辞要随机变化）：
"你是一个专业的角色扮演专家，请根据给定的世界观和角色设定，进行角色扮演。这是一个{题材/文风}背景的故事。用户扮演的角色为{用户角色}，你将扮演除了{用户角色}以外的所有角色，并负责剧情的推进。请以{人称}进行书写。"
"""

TASK_DESCRIPTION_PARAPHRASE_SYSTEM_PROMPT = """你是一个角色扮演任务描述生成器。

你的任务：
根据提供的信息，生成一个随机化的角色扮演任务描述（转述模式）。
...
2. 必须包含以下核心信息：
   ...
   - 说明使用第一人称还是第三人称书写
   - **根据世界观设定，推断并说明作品的题材和文风**（例如：中国都市、仙侠、武侠、异世界、古装、现代都市、轻小说、西方幻想、校园等）
   - **重要**：AI需要生成至少{最少字数}字的文本内容
   - **重要**：用户提供的对话内容需要自然地融入到生成的文本中
3. **重要警告**：绝对不能搞反角色分配！
   - 错误示例："你将化身为{用户角色}"（这是错误的！AI不应该扮演用户角色）
   - 错误示例："我将扮演其他角色"（这是错误的！用户不应该扮演其他角色）
   - 正确示例："用户扮演{用户角色}，你扮演其他角色"
...
正确格式示例（但措辞要随机变化）：
"你是一个专业的角色扮演专家，请根据给定的世界观和角色设定，进行角色扮演。这是一个{题材/文风}背景的故事。用户扮演的角色为{用户角色}，你将扮演除了{用户角色}以外的所有角色，并负责剧情的推进。请以{人称}进行书写。你需要生成至少{最少字数}字的文本内容，用户提供的对话内容需要自然地融入到生成的文本中。"
"""

TASK_DESCRIPTION_STRICT_PARAPHRASE_SYSTEM_PROMPT = """你是一个角色扮演任务描述生成器。

你的任务：
根据提供的信息，生成一个随机化的角色扮演任务描述（严格转述模式）。
...
2. 必须包含以下核心信息：
   ...
   - **重要**：AI需要生成至少{最少字数}字的文本内容
   - **重要**：用户提供的对话内容需要自然地融入到生成的文本中
   - **重要**：用户角色的发言严格限定在用户提供的内容范围内
   - **重要**：AI不能为用户角色生成用户没有说过的对话
3. **重要警告**：绝对不能搞反角色分配！
   - 错误示例："你将化身为{用户角色}"（这是错误的！AI不应该扮演用户角色）
   - 错误示例："我将扮演其他角色"（这是错误的！用户不应该扮演其他角色）
   - 正确示例："用户扮演{用户角色}，你扮演其他角色"
...
正确格式示例（但措辞要随机变化）：
"你是一个专业的角色扮演专家，请根据给定的世界观和角色设定，进行角色扮演。这是一个{题材/文风}背景的故事。用户扮演的角色为{用户角色}，你将扮演除了{用户角色}以外的所有角色，并负责剧情的推进。请以{人称}进行书写。你需要生成至少{最少字数}字的文本内容，用户提供的对话内容需要自然地融入到生成的文本中。用户角色的发言严格限定在用户提供的内容范围内，AI不能为用户角色生成用户没有说过的对话。"
"""
...
5. **重要**：思考过程中禁止使用"我"这个人称代词来指代任何角色，必须使用角色的本名或者上下文中的称呼，可以使用合适的第三人称人称代词来避免文本的冗长。例如：不要说"我心里想..."，而要说"张三心里想..."；"
6. **重要**：思考过程应该描述"如何推导出这个目标回应"，而不是直接引用或复述目标回应的内容。例如：不要说"所以张三说'你好'"，而是要说"张三观察到...，因此决定...，他的意图是..."
...
class ChatMLConverter:
    ...
    def generate_task_description(
        ...
        Args:
            user_role: 用户扮演的角色名称
            is_first_person: 是否使用第一人称
            mode: 模式选择，可选 "normal", "paraphrase", "strict-paraphrase"
            min_chars: 最少字数要求
        ...
        if mode == "paraphrase":
            system_prompt = TASK_DESCRIPTION_PARAPHRASE_SYSTEM_PROMPT.replace("{最少字数}", str(min_chars))
            default_template = f"你是一个专业的角色扮演专家，请根据给定的世界观和角色设定，进行角色扮演。用户扮演的角色为{user_role}，你将扮演除了{user_role}以外的所有角色，并负责剧情的推进。请以{person}进行书写。你需要生成至少{min_chars}字的文本内容，用户提供的对话内容需要自然地融入到生成的文本中。"
        elif mode == "strict-paraphrase":
            system_prompt = TASK_DESCRIPTION_STRICT_PARAPHRASE_SYSTEM_PROMPT.replace("{最少字数}", str(min_chars))
            default_template = f"你是一个专业的角色扮演专家，请根据给定的世界观和角色设定，进行角色扮演。用户扮演的角色为{user_role}，你将扮演除了{user_role}以外的所有角色，并负责剧情的推进。请以{person}进行书写。你需要生成至少{min_chars}字的文本内容，用户提供的对话内容需要自然地融入到生成的文本中。用户角色的发言严格限定在用户提供的内容范围内，AI不能为用户角色生成用户没有说过的对话。"
        else:
            system_prompt = TASK_DESCRIPTION_SYSTEM_PROMPT
            default_template = f"你是一个专业的角色扮演专家，请根据给定的世界观和角色设定，进行角色扮演。用户扮演的角色为{user_role}，你将扮演除了{user_role}以外的所有角色，并负责剧情的推进。请以{person}进行书写。"
        
        user_prompt = f"请生成一个角色扮演任务描述。\n\n用户扮演的角色：{user_role}\n人称：{person}\n"
        if mode in ("paraphrase", "strict-paraphrase"):
            user_prompt += f"最少字数：{min_chars}字\n"
...
def collect_messages_batch(
    messages: List[Dict[str, Any]],
    start_index: int,
    min_chars: int
) -> Tuple[List[Dict[str, Any]], int]:
    """从指定位置开始收集消息，直到满足最小字符数且最后一条是assistant。
    ...
    if collected:
        last_role = collected[-1].get("role", "")
        if last_role != "assistant":
            print(f"  警告：收集到的消息最后一条不是assistant（起始位置: {start_index}）")
    
    return collected, current_index
...
def convert_to_paraphrase_mode(
    messages: List[Dict[str, Any]],
    min_chars: int
) -> List[Dict[str, Any]]:
    """将消息转换为转述模式。
    ...
    转述模式规则：
    1. 多轮处理：循环调用 collect_messages_batch 直到所有消息处理完毕
    2. 每一轮：
       - 筛选所有user消息，取头1-2条
       - 去掉引号，用换行符拼接作为最终的一条user消息
       - 所有消息的完整content按顺序拼接作为最终的一条assistant消息
    3. 最终结构为 [user, assistant, user, assistant, ...]
    4. 不调用 adjust_first_message_role
    ...
    while start_index < len(messages):
        batch, next_index = collect_messages_batch(messages, start_index, min_chars)
        
        if not batch:
            break
        
        user_messages = [msg for msg in batch if msg.get("role") == "user"]
        
        if user_messages:
            user_count = min(2, len(user_messages))
            selected_users = user_messages[:user_count]
            
            user_contents = []
            for um in selected_users:
                content = um.get("content", "")
                cleaned = remove_quotes(content)
...
def convert_to_strict_paraphrase_mode(
    messages: List[Dict[str, Any]],
    min_chars: int
) -> List[Dict[str, Any]]:
    """将消息转换为严格转述模式。
...
def process_json_file(
    ...
    Args:
        ...
        book_name: 书籍名称
        output_dir: 输出目录
        converter: ChatMLConverter实例
        mode: 处理模式，可选 "normal", "paraphrase", "strict-paraphrase"
        min_chars: 每轮最小字符数（仅用于转述模式）
    ...
    task_description = converter.generate_task_description(user_role, is_first_person, world_settings, mode, min_chars)
    ...
    if mode == "paraphrase":
        processed_messages = convert_to_paraphrase_mode(merged_messages, min_chars)
    elif mode == "strict-paraphrase":
        processed_messages = convert_to_strict_paraphrase_mode(merged_messages, min_chars)
    else:
        processed_messages = adjust_first_message_role(merged_messages)
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default='paraphrase',
        choices=['normal', 'paraphrase', 'strict-paraphrase'],
        help='处理模式：normal（普通模式）、paraphrase（转述模式）、strict-paraphrase（严格转述模式）（默认：normal）'
    )
    parser.add_argument(
        '--min-chars',
        type=int,
        default=200,
        help='每轮最小字符数（仅用于转述模式，默认：500）'
    )
