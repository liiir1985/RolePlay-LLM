...
class DialogueLineAnnotation(BaseModel):
    is_dialog: bool = Field(description="是否是对话或者包裹在（）中的心理活动内容")
    speaker: str = Field(description="说话者的本名，如果是旁白则为空字符串")


class DialogueAnnotationResponse(BaseModel):
    annotations: List[DialogueLineAnnotation] = Field(description="每行文本的标注结果，数组长度必须与输入行数完全一致")
...
输出要求：
1. 必须输出JSON格式，包含一个annotations数组
2. 数组长度必须与输入的文本行数完全一致
3. 每个元素包含：
   - is_dialog: boolean，是否是对话或心理活动
   - speaker: string，说话者的本名，旁白则为空字符串
...
{text}"""
    ...
    def annotate_lines(
        ...
        pov_info: str,
        context_summary: str
    ) -> Optional[List[DialogueLineAnnotation]]:
        """
        标注文本行，判断每行是否是对话以及说话者。
        分批处理，每批batch_size行。
        """
        all_annotations: List[DialogueLineAnnotation] = []
        ...
        for batch_start in range(0, total_lines, self.batch_size):
            ...
            # 组装用户提示
            user_prompt = f"""请分析以下文本行，判断每行是否是对话或心理活动，并识别说话者。
...
请输出JSON格式的annotations数组，长度必须为{batch_size_actual}。"""
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    # 验证长度
                    if len(response.annotations) != batch_size_actual:
                        raise ValueError(
                            f"LLM返回的标注数量({len(response.annotations)})与输入行数({batch_size_actual})不一致"
                        )

                    all_annotations.extend(response.annotations)
...
def merge_annotated_lines(
    lines: List[str],
    annotations: List[DialogueLineAnnotation]
) -> List[Dict[str, Any]]:
    """
    合并连续相同speaker的行。
    合并规则：
    ...
    for line, annotation in zip(lines, annotations):
        ...
        if current_speaker is None:
            ...
            need_new = True
        elif current_is_dialog != is_dialog:
            need_new = True
        ...
        if need_new:
            ...
            if current_speaker is not None and current_content:
                result.append({
                    ...
                    "is_dialog": current_is_dialog,
                    "content": "\n".join(current_content)
                })
            # 开始新记录
            current_speaker = speaker
            current_is_dialog = is_dialog
