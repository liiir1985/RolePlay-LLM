...
first_person_name表示第一人称"我"对应的角色称呼。如果不是第一人称视角，或无法从文本中判断"我"是谁，则为空字符串。如果能判断，必须使用已知角色列表或本次提取出的角色称呼中的其中一个。
...
分析："铁拳"是"张三"的绰号→同一角色。"李四"是不同角色。第三人称叙述。
输出：{"characters": [["张三", "铁拳"], ["李四"]], "is_first_person": false, "first_person_name": ""}
...
请按照规则提取角色名称，输出JSON对象。"""
        ...
        try:
            ...
            if not isinstance(response, CharacterExtractionResponse):
                return SegmentExtractionResult(characters=[])
            
            is_first_person = response.is_first_person
            first_person_name = response.first_person_name.strip()
