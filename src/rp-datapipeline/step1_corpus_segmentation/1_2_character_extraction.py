...
核心规则：
1. 根据上下文语义判断名称归属。同一角色的不同称呼（姓名、姓氏、名字、外号、绰号等）放入同一子数组，不同角色放入不同子数组。注意：仅仅同时出现在文本中不代表是同一角色，必须有明确的语义关联。
2. 只输出有新称呼的角色。已知称呼无变化的不要输出。
3. 已知角色出现新称呼时，输出中必须包含至少一个已知称呼+新称呼。
...
文本："浅宫和冬马走进教室，七濑正在看书。"
分析："浅宫"是"浅宫美来"的姓氏→已知角色新称呼，附带已知称呼。"冬马"无新称呼→不输出。"七濑"是"七濑春辉"的姓氏→新称呼，附带已知称呼。
输出：{"characters": [["浅宫美来", "浅宫"], ["七濑春辉", "七濑"]]}
